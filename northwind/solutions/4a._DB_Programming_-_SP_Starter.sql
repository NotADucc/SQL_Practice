...
*/
CREATE OR ALTER PROCEDURE DetailsCategory @cId INT = 1
AS
BEGIN
	DECLARE @name NVARCHAR(255), @numberOfProducts INT = 0, @avgPrice FLOAT = 0.0;

	SELECT @name = CategoryName FROM Categories WHERE CategoryID = @cId;
	SELECT @numberOfProducts = COUNT(*) FROM Categories c JOIN Products p ON c.CategoryID = p.CategoryID WHERE c.CategoryID = @cId;
	SET @avgPrice = (SELECT AVG(p.UnitPrice) FROM Categories c JOIN Products p ON c.CategoryID = p.CategoryID WHERE c.CategoryID = @cId);

	IF @name IS NULL
	BEGIN
		RAISERROR ('This category doesn''t exist',18,1)
		RETURN
	END

	PRINT 'CategoryName = ' + @name
	PRINT 'Number of products = ' + STR(@numberOfProducts)
	PRINT 'Average Unit price = ' + STR(@avgPrice)
END

EXEC DetailsCategory 5
...
CREATE OR ALTER PROCEDURE ContactCustomers @companyName NVARCHAR(40), @numberOfCustomers INT OUTPUT
AS
BEGIN
	IF @companyName IS NULL
	BEGIN
		RAISERROR('company name is null', 18, 1)
		RETURN
	END

	IF NOT EXISTS(SELECT * FROM Suppliers WHERE CompanyName = @companyName)
	BEGIN
		RAISERROR('company name does''t exist', 18, 1)
		RETURN
	END
	
	IF (SELECT COUNT(*) FROM Suppliers WHERE CompanyName = @companyName) > 1
	BEGIN
		RAISERROR('there are multiple suppliers with this name', 18, 1)
		RETURN
	END
	...
	SET @numberOfCustomers = (
		...
		JOIN Products p ON od.ProductID = p.ProductID AND p.SupplierID = (SELECT SupplierID FROM Suppliers WHERE CompanyName = @companyName)
		WHERE DATEDIFF(MONTH, o.OrderDate, '2018-10-21') < 6
	);
END

DECLARE @numberOfCustomersOUT INT = 0;
EXEC ContactCustomers 'Refrescos Americanas LTDA', @numberOfCustomersOUT OUTPUT
